...
def test_bonus_play_strategy_play_wo_bonus(empty_stacks, normal_hand):
    ...
    assert any(np.array_equal(expected_stack, stack.to_array()) for stack in actual_stacks)

def test_bonus_play_strategy_play_with_bonus(empty_stacks):
    actual_player, actual_stacks = bonus_play_strategy(np.array([2, 3, 7, 40, 50, 55]), empty_stacks)
...
def test_bonus_play_strategy_play_play_entire_hand(midgame_stacks):
    actual_player, actual_stacks = bonus_play_strategy(np.array([38, 27, 17, 7, 8, 12]), midgame_stacks)
...
def test_gemini_strategy_plays_reasonable(midgame_stacks):
    actual_player, actual_stacks = gemini_strategy(np.array([21, 22]), stacks=midgame_stacks)
